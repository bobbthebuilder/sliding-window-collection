...
void testInvalidInstantiation()
{
    ...
    // case: input is empty
    BOOST_CHECK_THROW(sliding_window_collection::SimpleSort s(input, 1), std::invalid_argument);

    input = std::make_shared<std::vector<double>>(std::vector<double>({1.0, 2.0, 3.0, 4.0}));

    // case: window size is smaller than 1
    BOOST_CHECK_THROW(sliding_window_collection::SimpleSort s(input, 0), std::invalid_argument);

    // case: window size exceeds input size
    BOOST_CHECK_THROW(sliding_window_collection::SimpleSort s(input, 5), std::invalid_argument);
}

void testWindowEqualsInputSize()
{
    auto input = std::make_shared<std::vector<double>>(std::vector<double>({1.0, 2.0, 3.0, 4.0}));
    sliding_window_collection::SimpleSort s(input, 4);
    {
...
void testStep()
{
    ...
    sliding_window_collection::SimpleSort s(input, 5);
    {
        const auto result = s.get();
...
{
    {
        ...
        sliding_window_collection::SimpleSort s(input, 3);
    ...
    }
    {
        ...
        sliding_window_collection::SimpleSort s(input, 1);
        auto result = s.get();
        for (;!s.last();)
        {
            s.next();
